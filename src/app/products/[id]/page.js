    ...
    <main dir="rtl" className="p-6 md:p-10">
      <div className="flex flex-col md:flex-row items-center gap-6">
        <div className="relative w-full max-w-[300px] aspect-square">
          <Image src={product.image} alt={product.name} fill className="object-contain rounded-xl shadow-sm" />
        </div>
        <div className="max-w-xl text-right">
          ...
          <p className="font-bold text-lg md:text-2xl mb-4">السعر: {product.price} {product.currency}</p>
          <button onClick={handleAddToCart} className="mt-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg shadow">
            أضف إلى السلة
          </button>
        </div>
      </div>
